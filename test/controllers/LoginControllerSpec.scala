...
class LoginControllerSpec extends PlaySpec with Mockito{
  ...
  "display Login Form" in {
    val controller = getMockedObject
    when(controller.mockedUserForm.loginForm) thenReturn {val form = new UserForm{}
      form.loginForm}
    val result = controller.loginController.login().apply(FakeRequest().withCSRFToken)
  ...
  def getMockedObject: TestObj = {
    ...
    val controller = new LoginController(stubControllerComponents(),mockedUserForm,mockedUserInfoRepository)

    TestObj(stubControllerComponents(),mockedUserForm,controller,mockedUserInfoRepository)
  }

  case class TestObj(controllerComponent: ControllerComponents,mockedUserForm:UserForm,loginController: LoginController
